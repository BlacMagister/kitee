...
def reset_daily_interactions():
    return {
        ...
        "interactions": {}  # nantinya setiap wallet akan ditambahkan
    }

# Memuat log interaksi harian dari file, dan pastikan semua wallet ada di dalamnya
...
def get_random_questions_by_topic(topic, count):
    try:
        ...
        if len(available) < count:
            # Jika jumlah pertanyaan kurang, gunakan random.choices (dengan pengulangan)
...
def send_question_to_agent(agent_id, question):
    # URL dibangun berdasarkan agent_id (sesuaikan endpoint jika diperlukan)
    ...
    for attempt in range(3):  # Coba hingga 3 kali
        try:
            ...
            if response.status_code == 200:
                # Ambil jawaban dari response (sesuaikan parsing jika format berbeda)
                return response.json().get("choices", [{}])[0].get("message", "Jawaban tidak tersedia.")
...
def main():
    ...
    for wallet_index, wallet in enumerate(wallets, start=1):
        ...
        for agent_id, agent_info in agents.items():
            ...
            # Ambil sejumlah pertanyaan acak sesuai sisa interaksi yang diperbolehkan
...
# Jalankan script secara terus-menerus
